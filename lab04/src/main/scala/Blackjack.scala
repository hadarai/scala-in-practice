...
class Blackjack (deck: Deck) {
  ...
  val sizeOfDeck = blackjackDeck.deckOfCards.size
  // Points calculation:
    ...
    for (i <- 0 until n) {
      ...
      if (currentCard.value == ace){
        ...
        val points = currentCard.value match {
             ...
             case _: nine.type =>  9
             case _: ten.type =>  10
             case _: queen.type |_: jack.type | _: king.type => 10
           }
...
//      println(el.value)
  ...
  private def points11(groupOfCards: List[Card]): Int = {
    var sumOfPoints = 0
    for (el <- groupOfCards) {
      //      println(el.value)
      el.value match {
        ...
        case _:queen.type => sumOfPoints += 10
        case _:king.type => sumOfPoints += 10
        case _:ace.type => sumOfPoints += 11
      }
    }
    sumOfPoints
  }
  ...
  lazy val all21: List[List[Card]] = {
      for (amountOfCardsInCombination <- List.range(2, sizeOfDeck))
      yield {
        (for {cardCombination <- blackjackDeck.deckOfCards.combinations(amountOfCardsInCombination)
              if points1(cardCombination) == 21 || points11(cardCombination) == 21}
          yield {
            cardCombination
          }).toList.flatten
      }
...
object Blackjack {
  def apply(numOfDecks: Int):Blackjack = {
    val listOfListsOfCards = for (i <- List.range(0, numOfDecks)) yield {
        Random.shuffle(
          for {
